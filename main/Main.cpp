...
Matrix M;
void UpdateInput(Mesh *m, Matrix cameraV)
{
	if (IS_KEY_DOWN('A'))
	{
		moveVector.x -= 0.01f;
...
void Update(Window *w, Matrix cameraV);
...
// 	phongShader->frag.baseTex = tessellation;
// 	phongShader->frag.depthBuffer = depthBuffer;
	...
	planeShader = new PhongShader(PhongVert(), PhongFrag(tessellation, depthBuffer));;
...
void Update(Window *w, Matrix cameraV)
{
	...
	{
		UpdateInput(currentMesh,cameraV);
